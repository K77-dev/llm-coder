  ...
  describe('start()', () => {
    it('should spawn process and transition to starting then running', async () => {
      ...
      expect(mockSpawn).toHaveBeenCalledWith(
        ...
        ['-m', '/models/test.gguf', '--port', '9999', '-c', String(8192), '--ubatch-size', String(8192), '--batch-size', String(8192)],
  ...
  describe('restart()', () => {
    ...
    it('should accept port as dynamic config parameter', async () => {
      ...
      expect(mockSpawn).toHaveBeenLastCalledWith(
        '/usr/local/bin/llama-server',
        ['-m', '/models/model-a.gguf', '--port', '7777', '-c', String(8192), '--ubatch-size', String(8192), '--batch-size', String(8192)],
        { stdio: ['ignore', 'pipe', 'pipe'] },
      );
    });
    ...
    it('should accept execPath as dynamic config parameter', async () => {
      ...
      expect(mockSpawn).toHaveBeenLastCalledWith(
        ...
        ['-m', '/models/model-a.gguf', '--port', '9999', '-c', String(8192), '--ubatch-size', String(8192), '--batch-size', String(8192)],
        { stdio: ['ignore', 'pipe', 'pipe'] },
      );
    });
    ...
    it('should accept both port and execPath as dynamic config', async () => {
      ...
      expect(mockSpawn).toHaveBeenLastCalledWith(
        '/custom/llama',
        ['-m', '/models/model-a.gguf', '--port', '5555', '-c', String(8192), '--ubatch-size', String(8192), '--batch-size', String(8192)],
        { stdio: ['ignore', 'pipe', 'pipe'] },
      );
    });
  });
